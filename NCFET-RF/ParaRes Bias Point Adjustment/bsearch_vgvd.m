function [I_ds_final,Vd_prime_final,Vs_prime_final] = bsearch_vgvd(Vd_app,Vg_app,Rd,Rs,fet_arr,info_struct)
...
Vg_array=info_struct.Vg_array;
Vd_array=info_struct.Vd_array;
...
for i=1:length(Vd_app)
    ...
    for j=1:length(Vg_app)
        Vg=Vg_app(j);
        I_calc=213;
        ub=interp2(Vg_array,Vd_array,fet_arr,max(Vg_array),max(Vd_array));
        lb=min(min(fet_arr));
        I_guess=0.5*(ub+lb);
        ...
        while abs(I_guess-I_calc)>I_guess*1e-6
            ...
            if Vgs_calc<min(Vg_array)
               Vgs_calc=min(Vg_array);
            end
            
            if Vds_calc<min(Vd_array)
                Vds_calc=min(Vd_array);
            end
            
            I_calc=interp2(Vg_array,Vd_array,fet_arr,Vgs_calc,Vds_calc);
            ...
            elseif I_guess-I_calc==0
                ...
                disp(I_guess);
                disp(I_calc);
                disp('You may be out of range - assigned NaN');
                I_guess=NaN;
                break;
            end
            bsc=bsc+1;
